...
#define PWM_MAX_DUTY        80     /* Maximale Ausgangsleistung in Prozent */
    ...
    while (1)
    {
        if (rc_new_data)
        {
            ...
            if (linear_mode)
            {
                ...
                /* 2000us -> pwm_duty = PWM_MAX_DUTY   -> LED AN (max)        */
                /*------------------------------------------------------------*/
                ...
                else
                {
                    ...
                    pwm_duty = (uint8_t)(((uint32_t)(rc_pulse_us - RC_MIN_US) * PWM_MAX_DUTY)
                                         / (RC_MAX_US - RC_MIN_US));
                }
            }
            else
            {
                ...
                /* >= 1500us -> pwm_duty = PWM_MAX_DUTY -> LED AN (max)       */
                /*------------------------------------------------------------*/
        ...
        if (signal_timeout)
        {
            ...
            /* failsafe_high = 1 (PD6=GND) -> pwm_duty = PWM_MAX_DUTY -> LED AN (max) */
            /* failsafe_high = 0 (PD6=OFFEN) -> pwm_duty = 0 -> LED AUS   */
            /*------------------------------------------------------------*/
            pwm_duty = failsafe_high ? PWM_MAX_DUTY : 0;
        }
